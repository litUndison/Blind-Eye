...
{
    public string AIanswer = null; //отсюда забрать ответ
    ...
    private IEnumerator SendPromptRequestToGemini(string promptText)
    {
        ...
        using (UnityWebRequest www = new UnityWebRequest(url, "POST")){
            ...
            if (www.result != UnityWebRequest.Result.Success) 
            {
                ...
                GameObject.Find("Canvas").GetComponent<LoadingScript>()._state = Connect.error; //ошибка

            } 
            else {
                GameObject.Find("Canvas").GetComponent<LoadingScript>()._state = Connect.connected; //подключил
                ...
                Parameters.AddLog(Parameters._question, AIanswer);
            }
        }
    }
